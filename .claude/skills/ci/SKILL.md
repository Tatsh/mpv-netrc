...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `mpv_netrc/`, or dependency/version changes are **candidates**
...
For example, if a commit contains `main`, and `CHANGELOG.md`, the component is determined by
...
- Only Wiswa-managed files changed (workflows,
  `package.json`, `.pre-commit-config.yaml`, `.claude/agents/`, `.claude/rules/`,
