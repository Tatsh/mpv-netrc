...
### When Lua code is being committed

If any changed source files are being committed, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes.
  ...
  files under `mpv_netrc/`, or dependency/version changes. **Skip for**: workflows, CI config, `.claude/`,
...
For example, if a commit contains `mpv_netrc/main`, and `CHANGELOG.md`, the component is determined
by the source files only. `CHANGELOG.md` is simply staged alongside them.
...
- Only Wiswa-managed files changed (workflows,
  `package.json`, `.pre-commit-config.yaml`, `.claude/agents/`,
  `.cursor/rules/`, `.github/instructions/`, `CITATION.cff`,
  `.vscode/dictionary.txt`, `.wiswa.jsonnet`, etc.).
- No files under the primary module or source directories changed.
...
- Top-level config (`package.json`, `package.json`) → `project:`.
